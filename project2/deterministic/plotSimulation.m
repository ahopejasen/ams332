...
for theRun=1:numRuns
	...
	titleTxt4={['ICs: cI_{r}=',num2str(IC(vs.ir,theRun)),  ...
			'cI_{p}=',num2str(IC(vs.ip,theRun)), ...
			'  cro_{r}=',num2str(IC(vs.o_r,theRun)), ...
			'  cro_{p}=',num2str(IC(vs.op,theRun))]; ...
			['\mu=',num2str(theParms.P.muCI), ...
				'  \omega=', num2str(theParms.P.wCI), ...
			' \chi_{cI,R}=',num2str(theParms.P.xCIRna), ...
			'/ \chi_{cI,P }=',num2str(theParms.P.xCIPro), ...
			'  \chi_{cro}=',  num2str(theParms.P.xCroRna), ...
			'  k=',num2str(theParms.P.kCI)]};
	...
	if doLog
		...
		if (theParms.N.doOde45)
			...
			%% NOT WORKING :-(
			%thin data if needed <cite>https://www.mathworks.com/matlabcentral/newsreader/view_thread/167093</cite>
%			if size(t_ode_cells{theRun})>100
%				ii=round(linspace(1,(t_ode_cells{theRun}),100));
%			else
%				ii=[1:size(t_ode_cells{theRun})];
%			end
%			t_plot=t_ode_cells{theRun}([ii]);
%			y1_plot=y1{theRun}([ii],:);
%			semilogy(t_plot,y1_plot,'+')
